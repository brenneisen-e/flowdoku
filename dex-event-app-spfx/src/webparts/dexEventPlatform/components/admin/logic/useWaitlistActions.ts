...
import { idSequenceCheck, notifyPromotedFor } from './healAllEvents';
...
export function useWaitlistActions(ctx: UseWaitlistActionsCtx): UseWaitlistActionsResult {
   ...
   */
  // v30.70: Rumpf in logic/healAllEvents.ts (`notifyPromotedFor`) — die
  // Sammel-Heilung im Admin-Hub braucht denselben Mail-Aufbau, und zwei
  // Kopien wären beim nächsten Template-Wechsel auseinandergelaufen.
  const notifyPromoted = async (promoted: { email: string; name?: string }): Promise<void> => {
    if (!eventServiceRef || !selectedEvent) return;
    await notifyPromotedFor(eventServiceRef, allEvents, selectedEvent, promoted);
  };
  ...
  // v30.70: Rumpf in logic/healAllEvents.ts (`idSequenceCheck`) — die
  // Sammel-Heilung entscheidet damit, welche Events neu nummeriert werden.
  const recentCancellation = (regs: SPRegistration[]): { recent: boolean; whenIso: string; detail: string } => idSequenceCheck(regs);
