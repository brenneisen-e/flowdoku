  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <FlowNode
        ...
        details="Bei einer ECHT neuen Anmeldung (kein bestehendes Abgemeldet-Item) holt der Service die nächste TeilnehmerID atomar aus der Counter-Liste DEX_TeilnehmerCounter (eine Liste pro Subsite mit genau einem Item, NextValue=N). Vorgehen: GET counter-item mit ETag → PATCH NextValue=N+1 mit IF-MATCH=etag → bei HTTP 412 (jemand war schneller) wird mit kurzem Jitter retried (max 8x). Damit können mehrere User parallel anmelden, ohne dass IDs doppelt vergeben werden. Bei Reaktivierung eines Abgemeldet-Items wird der Counter NICHT angefasst — die TeilnehmerID bleibt null und der DEX_IDReorder-Flow vergibt sie sequentiell zurück. Fallback bei alten Events ohne Counter-Liste: max(TeilnehmerID)+1 — dann muss der Admin einmalig 'Spalten fixen' klicken, damit die Counter-Liste angelegt + mit dem aktuellen Max-Wert geseedet wird."
      />
      <Arrow />
      <FlowNode
        type="subprocess"
