            ...
            description: (
              <>
                ...
                <ul style={{ marginTop: 6, marginBottom: 6, paddingLeft: 18, lineHeight: 1.55 }}>
                  ...
                  <li>{isDe
                    ? 'Seit v18.73: Feldtyp „Datei-Upload". Der Teilnehmer wählt im Anmeldeformular eine Datei aus (PDF, Bild, Office-Dokument; max. 15 MB). Du kannst das Feld als Pflichtfeld markieren — dann blockiert die Anmeldung, bis eine Datei gewählt wurde. Nach erfolgreicher Anmeldung wird die Datei direkt an die Zeile des Teilnehmers in der Teilnehmerliste als Dokument angehängt; der Dateiname erscheint zusätzlich in der Antwort-Spalte. Im Admin Center öffnest du die Dateien pro Teilnehmer über den „Datei"-Button. Hinweis: greift bei der normalen Anmeldung (auch „für andere anmelden"); bei Team-Anmeldungen und bei Sub-Event-eigenen Feldern ist der Datei-Upload aktuell noch nicht aktiv.'
                    : 'Since v18.73: field type "File upload". The attendee picks a file in the registration form (PDF, image, Office document; max. 15 MB). You can mark the field as required — registration is then blocked until a file is chosen. After a successful registration the file is attached directly to the attendee\'s row in the participant list as a document; the file name also appears in the answer column. In the Admin Center you open the files per attendee via the "File" button. Note: works for standard registration (incl. "register on behalf of"); for team registrations and sub-event-specific fields the upload is not active yet.'}</li>
                  <li>{isDe
