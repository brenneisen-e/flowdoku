        ...
        steps: [
            ...
            description: (
              <>
                ...
                <ul style={{ marginTop: 6, marginBottom: 6, paddingLeft: 18, lineHeight: 1.55 }}>
                  ...
                  <li>{isDe
                    ? 'Seit v18.41: Bei People-Picker-Feldern (Typ „Person") gibt es das Häkchen „Ausgewählte Person bei An-/Abmelde-Mail auf CC setzen". Beispiel: ein Feld „Assistenz" — die dort angegebene Person bekommt die Anmelde- und Abmelde-Mail automatisch in Kopie (CC). Wirkt nur auf die Mails, NICHT auf den Outlook-Termin (die Person wird nicht zum Kalendereintrag eingeladen).'
                    : 'Since v18.41: people-picker fields (type „Person") offer a checkbox „CC the selected person on registration/cancellation email". Example: an „Assistant" field — the person entered there automatically receives the registration and cancellation email in CC. Affects emails only, NOT the Outlook event (the person is not invited to the calendar entry).'}</li>
                  <li>{isDe
                  ...
                  <li>{isDe
                    ? 'Seit v18.75: ganz unten die Section „Sicherheitshinweis vor dem Absenden anzeigen?" (Default aus). Aktiv erscheint nach dem „Anmelden"-Klick — und vor der eigentlichen Anmeldung — ein Bestätigungs-Dialog. Zwei Varianten: „Auswahl-Übersicht" listet Haupt-Event und gewählte Sub-Events; der Teilnehmer kann einzelne Punkte vor dem Absenden noch ab- oder zuwählen. „Eigener Hinweistext" zeigt stattdessen einen frei formulierten Hinweis (z.B. zur Verbindlichkeit oder zu Storno-Fristen), den der Teilnehmer per Checkbox bestätigen muss.'
                    : 'Since v18.75: at the very bottom a section „Show a confirmation prompt before submitting?" (default off). When enabled, a confirmation dialog appears after clicking „Register" — and before the actual registration. Two variants: „Selection summary" lists the main event and selected sub-events; the attendee can de-/select items before submitting. „Custom hint text" instead shows a free-text note (e.g. about binding registration or cancellation deadlines) the attendee must acknowledge via checkbox.'}</li>
                  <li>{isDe
                    ? 'Seit v19.0: Feldtyp „Dokument (PDF/Bild-Upload)". Damit bittest du Teilnehmer um einen Datei-Upload (PDF, JPG, PNG, max. 10 MB) — z.B. einen Nachweis. Die Datei wird an die Teilnehmer-Zeile angehängt (keine Spalte) und ist im Admin Center pro Teilnehmer abrufbar. Wie bei anderen Feldern als Pflichtfeld konfigurierbar. Der Teilnehmer kann die Datei direkt bei der Anmeldung oder später über „Meine Events" hochladen, ersetzen oder löschen.'
                    : 'Since v19.0: field type „Document (PDF/image upload)". Use it to ask attendees for a file upload (PDF, JPG, PNG, max. 10 MB) — e.g. a proof document. The file is attached to the participant row (no column) and can be opened per participant in the Admin Center. Configurable as required like other fields. Attendees can upload, replace or delete the file directly at registration or later via „My Events".'}</li>
                </ul>
                {isDe
          ...
          },
          {
            ...
            description: (
              <>
                {isDe
                  ? 'Stelle die E-Mail-Sprache ein (DE oder EN), hinterlege einen optionalen Event-Logo für den Mail-Header, passe die Outlook-Terminbeschreibung an und editiere falls nötig die vier System-Templates: Anmeldung, Warteliste, Abmeldung, Nachrücken. Über den Schalter „Bestätigungs-E-Mails an Teilnehmer schicken" steuerst du, ob überhaupt Mails rausgehen. Ab v19.21 kannst du darunter zusätzlich getrennt einstellen, ob die Anmelde-Bestätigung und die Abmelde-Bestätigung verschickt werden — so kannst du z.B. nur die Abmelde-Mail abschalten, wenn du Teilnehmer still abmeldest, und die Anmelde-Bestätigung trotzdem aktiv lassen. Ab v19.22 lässt sich das auch pro Sub-Event einstellen (eigener Tab oben). Ganz oben im Reiter zeigt eine Übersichts-Box (v19.23) auf einen Blick, was für den gewählten Tab automatisch kommuniziert wird und was nicht. Optional (v19.23, ab v19.24 auch pro Sub-Event) lässt sich aktivieren, dass eine Outlook-Absage automatisch eine Abmeldung vom Event auslöst — das wird im Outlook-Absage-Verarbeitungsschritt gesteuert und greift erst nach der einmaligen Einrichtung im Tenant. Darunter findest du die Sub-Event-Verwaltung (Trainingssessions etc.). Ab v11.57: hat dein Event Sub-Events, kannst du oben über die Tab-Leiste pro Sub-Event eine eigene Mail-Sprache, eigene Outlook-Termin-Texte und eigene Logos pflegen — ideal z.B. wenn ein Sub-Event auf Deutsch und eines auf Englisch läuft.'
                  : 'Set the email language (DE or EN), upload an optional event logo for the email header, customize the Outlook invite description, and edit the four system templates if needed: Registration, Waitlist, Cancellation, Promotion. The "Send confirmation emails to attendees" switch controls whether any mails go out at all. Since v19.21 you can additionally toggle the registration confirmation and the cancellation confirmation separately below it — so you can e.g. turn off only the cancellation email when you remove attendees silently, while keeping the registration confirmation active. Since v19.22 this can also be set per sub-event (its own tab on top). At the very top of the tab an overview box (v19.23) shows at a glance what is communicated automatically for the selected tab and what is not. Optionally (v19.23, since v19.24 also per sub-event) you can enable that an Outlook decline automatically triggers deregistration from the event — this is driven by the Outlook-decline processing step and only takes effect after the one-time setup in the tenant. Below you find the sub-event (training session) management. Since v11.57: if the event has sub-events, you can use the tab bar on top to maintain separate language, Outlook texts and logos per sub-event — handy e.g. when one sub-event runs in German and another in English.'}
              </>
            ),
