    ...
    perspectives: [
      ...
      },
      {
        ...
        steps: [
          ...
          {
            number: 8,
            title: isDe ? 'Weg B: Scannen, Bestätigung & Live-Ergebnis (grün / blau / rot)' : 'Path B: scan, confirmation & live result (green / blue / red)',
            description: (
              <>
                {isDe
                  ? 'Du hältst die Kamera auf den QR-Code — die Erkennung passiert sofort, ohne auf einen Auslöser zu warten. Ein erfolgreicher Scan checkt NICHT blind ein: Zuerst erscheint eine Bestätigungs-Karte mit Name, Job-Titel, Standort und Profilfoto der Person — erst mit deiner Bestätigung wird tatsächlich eingecheckt (so lassen sich Verwechslungen vermeiden). Je nach Zustand des Teilnehmers gibt es dann eine farbige Ergebnis-Karte oben in der Maske: GRÜN (success) = frisch eingecheckt (Status „Eingecheckt", Session-Counter erhöht sich um 1). BLAU (info) = war bereits eingecheckt (kein doppelter Counter-Sprung, nur Hinweis „Bereits eingecheckt"). ROT (error) = Code nicht zugeordnet — z.B. nicht registriert, abgemeldet oder QR eines anderen Events. Die Karte nennt dann den Grund. Der Live-Counter oben („7 …") zählt pro Scanner-Session lokal mit, damit du auf einen Blick siehst wie viele du schon durchgecheckt hast.'
                  : 'Point the camera at the QR code — recognition is instant, no shutter button. A successful scan does NOT check in blindly: first a confirmation card appears with the person\'s name, job title, location and profile photo — only after your confirmation is the person actually checked in (this avoids mixups). Depending on the attendee\'s state a colored result card then appears at the top: GREEN (success) = freshly checked in (status „Checked-in", session counter +1). BLUE (info) = was already checked in (no double counter bump, just „Already checked in"). RED (error) = code not recognized — e.g. not registered, cancelled, or a QR from a different event. The card names the reason. The live counter at the top („7 …") tracks per scanner session locally so you see at a glance how many you\'ve processed.'}
              </>
            ),
            mockup: (
              <AppPreview
