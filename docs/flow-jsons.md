...
|---|----------------|-----------------|----------------|--------|---------------|
| 1 | **NEU** | `Filter_Sort_Aktive` | Array filtern (Filter array) | Innerhalb der Schleife `Batch_Until_Clean`, direkt nach `Set_AllParticipants` (vor `Generate_Indices`) | 1. Schleife `Batch_Until_Clean` aufklappen, auf das **+** direkt unter `Set_AllParticipants` klicken → **Aktion hinzufügen**.<br>2. Nach **„Array filtern"** suchen (Kategorie Datenvorgang) und auswählen.<br>3. **Von:** über den fx-Tab eintragen: `variables('AllParticipants')`<br>4. Auf **„In erweitertem Modus bearbeiten"** klicken und eintragen: `@not(equals(item()?['Status']?['Value'], 'Warteliste'))`<br>5. ⋮ → **Umbenennen** → `Filter_Sort_Aktive` |
| 2 | **NEU** | `Filter_Sort_Warteliste` | Array filtern (Filter array) | Direkt nach `Filter_Sort_Aktive` | 1. **+** direkt unter `Filter_Sort_Aktive` → **Aktion hinzufügen** → **„Array filtern"**.<br>2. **Von** (fx): `variables('AllParticipants')`<br>3. **„In erweitertem Modus bearbeiten"** → `@equals(item()?['Status']?['Value'], 'Warteliste')`<br>4. ⋮ → **Umbenennen** → `Filter_Sort_Warteliste` |
| 3 | **NEU** | `Sort_AktiveZuerst` | Verfassen (Compose) | Direkt nach `Filter_Sort_Warteliste` | 1. **+** unter `Filter_Sort_Warteliste` → **Aktion hinzufügen** → **„Verfassen"**.<br>2. **Eingaben** (fx): `union(body('Filter_Sort_Aktive'), body('Filter_Sort_Warteliste'))`<br>3. ⋮ → **Umbenennen** → `Sort_AktiveZuerst` |
| 4 | **NEU** | `Set_AllParticipants_Sortiert` | Variable festlegen (Set variable) | Direkt nach `Sort_AktiveZuerst` | 1. **+** unter `Sort_AktiveZuerst` → **Aktion hinzufügen** → **„Variable festlegen"**.<br>2. **Name:** im Dropdown `AllParticipants` wählen.<br>3. **Wert** (fx): `outputs('Sort_AktiveZuerst')`<br>4. ⋮ → **Umbenennen** → `Set_AllParticipants_Sortiert` |
| 5 | **GEÄNDERT** | `Generate_Indices` | Run-after-Änderung (bestehende Compose-Action) | In `Batch_Until_Clean`, nach den neuen Actions 1–4 | 1. `Generate_Indices` anklicken → ⋮ → **Ausführen nach konfigurieren**.<br>2. Prüfen: es darf NUR `Set_AllParticipants_Sortiert` mit Haken **„ist erfolgreich"** drinstehen.<br>3. Falls `Set_AllParticipants` noch als Vorgänger drinsteht: Haken entfernen. (Beim Einfügen über die **+**-Punkte verdrahtet der Designer das meist schon automatisch.) |
| 6 | **NEU** | `Requeue_Reorder_N` | SharePoint — Element erstellen (Create item) | NEIN-Zweig von `Is_B2RunSplit` → `Check_Nachrücken` JA → `Condition_1` JA, als neue LETZTE Action nach `Audit_Cancelled_N` | 1. **+** direkt unter `Audit_Cancelled_N` → **Aktion hinzufügen** → SharePoint **„Element erstellen"**.<br>2. **Websiteadresse:** `https://deudeloitte.sharepoint.com/sites/DOL-c-DE-EventExperiencePlatform` (Root-Site wie der Trigger, NICHT die Subsite).<br>3. **Listenname:** `DEX_IDReorder`<br>4. **Title:** `Reorder: Folge-Korrektur nach Nachrücken` (als Text eintippen).<br>5. **EventId** (fx): `triggerOutputs()?['body/EventId']`<br>6. **EventNumber** (fx): `triggerOutputs()?['body/EventNumber']`<br>7. **SubsiteUrl** (fx): `triggerOutputs()?['body/SubsiteUrl']`<br>8. **Status:** `Pending` (falls ein Dropdown erscheint: „Benutzerdefinierten Wert eingeben" → `Pending`).<br>9. **CancelledName** (fx): `triggerOutputs()?['body/CancelledName']`<br>10. **CancelledEmail** (fx): `triggerOutputs()?['body/CancelledEmail']`<br>11. ⋮ → **Umbenennen** → `Requeue_Reorder_N`<br>12. ⋮ → **Ausführen nach konfigurieren** → bei `Audit_Cancelled_N` DREI Haken setzen: **ist erfolgreich + ist fehlgeschlagen + wird übersprungen** (die Audit-Kette ist best-effort — der Folge-Reorder muss trotzdem kommen). |
| 7 | **NEU** | `Requeue_Reorder_D` | SharePoint — Element erstellen (Create item) | JA-Zweig `Is_B2RunSplit` → `Check_Durchstarter_Free` JA → `Has_Durchstarter_Waitlist` JA, nach `Audit_Cancelled_D` | 1. **+** direkt unter `Audit_Cancelled_D` → **Aktion hinzufügen** → SharePoint **„Element erstellen"**.<br>2. Alle Felder und fx-Ausdrücke EXAKT wie Zeile 6, Schritte 2–10.<br>3. ⋮ → **Umbenennen** → `Requeue_Reorder_D`<br>4. ⋮ → **Ausführen nach konfigurieren** → bei `Audit_Cancelled_D` DREI Haken: **ist erfolgreich + ist fehlgeschlagen + wird übersprungen**. |
| 8 | **NEU** | `Requeue_Reorder_F` | SharePoint — Element erstellen (Create item) | JA-Zweig `Is_B2RunSplit` → `Check_Funstarter_Free` JA → `Has_Funstarter_Waitlist` JA, nach `Audit_Cancelled_F` | 1. **+** direkt unter `Audit_Cancelled_F` → **Aktion hinzufügen** → SharePoint **„Element erstellen"**.<br>2. Alle Felder und fx-Ausdrücke EXAKT wie Zeile 6, Schritte 2–10.<br>3. ⋮ → **Umbenennen** → `Requeue_Reorder_F`<br>4. ⋮ → **Ausführen nach konfigurieren** → bei `Audit_Cancelled_F` DREI Haken: **ist erfolgreich + ist fehlgeschlagen + wird übersprungen**. |
| 9 | **GEÄNDERT** | `Error_Handler` (bzw. `Set_Failed` darin) | Run-after-Änderung (bestehender Scope) | Top-Level, hängt an `DEX_IDReorder` | 1. `Error_Handler` anklicken → ⋮ → **Ausführen nach konfigurieren**.<br>2. Bei `DEX_IDReorder` zusätzlich zu **ist fehlgeschlagen** auch **wird übersprungen** anhaken.<br>3. Effekt: das Queue-Item wird auch bei Abbruch VOR dem Done-Patch auf `Failed` gesetzt statt ewig auf `Processing` zu bleiben. |
| 10 | **NEU (optional)** | `Check_Renumber_Clean` | Bedingung (Condition) | Top-Level zwischen `Batch_Until_Clean` und `Filter_Non_Waitlist` | 1. **+** zwischen `Batch_Until_Clean` und `Filter_Non_Waitlist` → **Aktion hinzufügen** → **„Bedingung"**.<br>2. Linke Seite (fx): `length(body('GenerateSPData'))`<br>3. Operator: **ist gleich** · rechte Seite: `0`<br>4. ⋮ → **Umbenennen** → `Check_Renumber_Clean` (`Filter_Non_Waitlist` bleibt NACH der Condition — verdrahtet der Designer automatisch). |
| 11 | **NEU (optional)** | `Set_Failed_Unclean` | SharePoint — Element aktualisieren (Update item) | Im NEIN-Zweig von `Check_Renumber_Clean` | 1. Im **NEIN**-Zweig **Aktion hinzufügen** → SharePoint **„Element aktualisieren"**.<br>2. **Websiteadresse:** Root-Site (wie Zeile 6, Schritt 2) · **Listenname:** `DEX_IDReorder`<br>3. **Id** (fx): `triggerOutputs()?['body/ID']`<br>4. **Title** (fx): `triggerOutputs()?['body/Title']` (sonst wird der Titel geleert).<br>5. **Status:** `Failed`<br>6. ⋮ → **Umbenennen** → `Set_Failed_Unclean` |
| 12 | **NEU (optional)** | `Terminate_Unclean` | Beenden (Terminate) | Im NEIN-Zweig, nach `Set_Failed_Unclean` | 1. **+** unter `Set_Failed_Unclean` → **Aktion hinzufügen** → **„Beenden"**.<br>2. **Status:** `Fehlgeschlagen` (Failed).<br>3. ⋮ → **Umbenennen** → `Terminate_Unclean` — verhindert, dass nach 5 erfolglosen Renummerier-Runden noch mit unfertigen IDs nachgerückt wird. |
...
> **Die exakte, click-by-click Schritt-für-Schritt-Anleitung steht unten unter
> „UI-Anleitung 2026-06-08 (v19.11) — Nachrück-Audit EXAKT".** Der Abschnitt hier
...
**Voraussetzung (einmal pro Event):** Im Admin Center **„Spalten fixen"**
ausführen — legt die drei Spalten auf der Teilnehmerliste an. Die App liefert im
Trigger-Item bereits `CancelledEmail` (E-Mail der abgemeldeten Person).

**Es gibt bis zu DREI Nachrück-Zweige** (je nach Event nur Zweig A nötig):
- **Zweig A — Normal:** Promote-Action `Promote_Waitlist`, Warteliste-Quelle `Get_Waitlist_First`.
- **Zweig B — B2Run Durchstarter:** `Promote_Durchstarter`, Quelle `Get_Waitlist_First_Durchstarter`.
...
3. **Site Address:** auf das Feld klicken → **„Enter custom value"** → **denselben
   ...
   die Promote-Action öffnen, den Site-Address-Wert markieren, kopieren, hier
   einfügen.)
4. **Method**, **Uri**, **Headers**, **Body** wie unten angegeben setzen.
   - **Ausdrücke** (alles in `@{ … }`): an die Stelle im Textfeld klicken, **fx**
     öffnen, den Ausdruck eintippen, **OK** — niemals als reinen Text tippen.
   - **Headers** ist ein Schlüssel/Wert-Gitter: pro Zeile links den Key, rechts den Wert.
5. **Umbenennen:** oben rechts an der Action auf **⋮ (drei Punkte) → „Rename"** →
   den angegebenen Namen exakt eingeben (die Namen werden in den Ausdrücken
   referenziert!).
6. **Robust machen:** **⋮ → „Configure run after"** → beim Vorgänger zusätzlich zu
   **„is successful"** auch **„has failed"** und **„is skipped"** anhaken. Damit
   läuft die Kette immer weiter, selbst wenn ein Feld mal fehlt (best-effort).
...
- **Body:**
  ```
  {
    "__metadata": { "type": "SP.Data.TeilnehmerListItem" },
    "PromotedDate": "@{utcNow()}",
...
- **Body:**
  ...
  {
    "__metadata": { "type": "SP.Data.TeilnehmerListItem" },
    "ReplacedByParticipantEmail": "@{first(body('Get_Waitlist_First')?['d']?['results'])?['ParticipantEmail']}"
  }
  ```
- **Rename** → `Audit_Cancelled_N`. **Run after** `Get_Cancelled_N` (+ has failed/skipped).
...
- **Body:**
  ...
  {
    "__metadata": { "type": "SP.Data.TeilnehmerListItem" },
    "ReplacedByParticipantEmail": "@{first(body('Get_Waitlist_First_Durchstarter')?['d']?['results'])?['ParticipantEmail']}"
  }
  ```
- **Rename** → `Audit_Cancelled_D`. **Run after** `Get_Cancelled_D` (+ has failed/skipped).
...
- **Body:**
  ...
  {
    "__metadata": { "type": "SP.Data.TeilnehmerListItem" },
    "ReplacedByParticipantEmail": "@{first(body('Get_Waitlist_First_Funstarter')?['d']?['results'])?['ParticipantEmail']}"
  }
  ```
- **Rename** → `Audit_Cancelled_F`. **Run after** `Get_Cancelled_F` (+ has failed/skipped).

---

**Wichtige Details, die oft schiefgehen:**
- **Listenname** ist überall `Teilnehmer` (kein Sonderzeichen → Item-Type
  `SP.Data.TeilnehmerListItem`).
- Die `Get_Waitlist_First*`- und `Get_Cancelled_*`-GETs laufen über `odata=verbose`
  → Ergebnis-Pfad ist **`?['d']?['results']`** (mit `Accept: …;odata=verbose`).
  Ohne diesen Accept-Header käme `value` statt `d/results` und `first(...)` liefert null.
- `PromotedDate` ist eine **DateTime**-Spalte — `utcNow()` (ISO-String) passt.
- **Reihenfolge wichtig:** `Audit_Promoted_*` referenziert `Get_Waitlist_First*`
  (die Action existiert im Zweig schon, weil der Promote sie nutzt).
  `Audit_Cancelled_*` referenziert `Get_Cancelled_*` (die Action davor).
- **Test:** Person A (Status Warteliste) + Event voll. Person B abmelden →
  Flowlauf prüfen: A bekommt `PromotedDate` = jetzt und `ReplacedParticipantEmail`
  = B; B bekommt `ReplacedByParticipantEmail` = A. In der App: aktive Tabelle
  „Nachgerückt am"/„Hat ersetzt" bei A, Abmeldungen-Tabelle „Wurde ersetzt durch"
  = A bei B.

Sobald im Tenant umgesetzt: Flow-JSON zurückschicken → finaler Stand wird hier
eingepflegt.
