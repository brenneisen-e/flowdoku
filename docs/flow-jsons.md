...
| 10 | **NEU (optional)** | `Check_Renumber_Clean` — Top-Level zwischen `Batch_Until_Clean` und `Filter_Non_Waitlist` | 1. **+** zwischen `Batch_Until_Clean` und `Filter_Non_Waitlist` → **Aktion hinzufügen** → **„Bedingung"** (Condition).<br>2. Linke Seite (fx): `length(body('GenerateSPData'))`<br>3. Operator: **ist gleich** · rechte Seite: `0`<br>4. ⋮ → **Umbenennen** → `Check_Renumber_Clean` (`Filter_Non_Waitlist` bleibt NACH der Condition — verdrahtet der Designer automatisch). |
| 11 | **NEU (optional)** | `Set_Failed_Unclean` — im NEIN-Zweig von `Check_Renumber_Clean` | 1. Im **NEIN**-Zweig **Aktion hinzufügen** → SharePoint **„Element aktualisieren"** (Update item).<br>2. **Websiteadresse:** Root-Site (wie Zeile 6, Schritt 2) · **Listenname:** `DEX_IDReorder`<br>3. **Id** (fx): `triggerOutputs()?['body/ID']`<br>4. **Title** (fx): `triggerOutputs()?['body/Title']` (sonst wird der Titel geleert).<br>5. **Status:** `Failed`<br>6. ⋮ → **Umbenennen** → `Set_Failed_Unclean` |
| 12 | **NEU (optional)** | `Terminate_Unclean` — im NEIN-Zweig, nach `Set_Failed_Unclean` | 1. **+** unter `Set_Failed_Unclean` → **Aktion hinzufügen** → **„Beenden"** (Terminate).<br>2. **Status:** `Fehlgeschlagen` (Failed).<br>3. ⋮ → **Umbenennen** → `Terminate_Unclean` — verhindert, dass nach 5 erfolglosen Renummerier-Runden noch mit unfertigen IDs nachgerückt wird. |
